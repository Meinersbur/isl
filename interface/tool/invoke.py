...
class Invoke:
    ...
    def __init__(self, cmd, *args, cwd=None, setenv=None, appendenv=None, stdout=None, stderr=None, std_joined=None, std_prefixed=None, stdout_append=None, stderr_append=None, std_joined_append=None, std_prefixed_append=None):
        ...
        self.stdout_append = Invoke.hlist(stdout_append)
        self.stderr_append = Invoke.hlist(stderr_append)
        self.std_joined_append= Invoke.hlist(std_joined_append)
        self.std_prefixed_append = Invoke.hlist(std_prefixed_append)

    def cmd(self,shell=False):
        # FIXME: Windows
        ...
        if shell:
            ...
            cmdargs = shjoin(args)
        if self.cwd is None:
            result = env + shortcmd + (' ' if args else '') + cmdargs
        else:
            result = '(cd ' + shquote(self.cwd) + ' && ' + env + shortcmd + (' ' if args else '') + cmdargs + ')'
        return result
    ...
    def execute(self, onerror=IGNORE,
                ...
                stdout=None, stderr=None, std_joined=None, std_prefixed=None, stdout_append=None, stderr_append=None, std_joined_append=None, std_prefixed_append=None, forcepopen=False, shell=False, encoding='utf-8'):
        ...
        stdout_append = self.stdout + Invoke.hlist(stdout_append)
        stderr_append = self.stderr + Invoke.hlist(stderr_append)
        std_joined_append = self.std_joined + Invoke.hlist(std_joined_append)
        std_prefixed_append = self.std_prefixed + Invoke.hlist(std_prefixed_append)
        ...
        def execute_run():
            if print_command:
                sys.stdout.flush()
                print('$', self.cmd(shell=shell), file=sys.stderr)
                sys.stderr.flush()
            ...
            ret = subprocess.run(cmdline, cwd=cwd, env=env, check=(
                onerror == Invoke.EXCEPTION), stdout=stdout_mode, stderr=stderr_mode, universal_newlines=True, shell=shell, encoding=encoding)
            ...
            if print_exitcode:
                exitmsg = "Exit with code {exitcode} in {walltime}".format(
                    exitcode=exitcode, walltime=walltime)
                sys.stdout.flush()
        ...
        def execute_popen():
            ...
            if print_command or return_prefixed or print_prefixed or std_prefixed:
                command = self.cmd()
            ...
            with contextlib.ExitStack() as stack:
                ...
                def open_or_handle(arg,append=False):
                    if isinstance(arg, str):
                        # Finally close
                        if append:
                            return stack.enter_context(open(file, 'a'))
                        else:
                            return stack.enter_context(open(file, 'w'))
                    return arg

                for file in stdout:
                    stdouthandles.append(open_or_handle(file))
                for file in stdout_append:
                    stdouthandles.append(open_or_handle(file,append=True))
                for file in stderr:
                    stderrhandles.append(open_or_handle(file))
                for file in stderr_append:
                    stdouthandles.append(open_or_handle(file,append=True))
                for file in std_joined:
                    h = open_or_handle(file)
                    stdouthandles.append(h)
                    stderrhandles.append(h)
                for file in std_joined_append:
                    h = open_or_handle(file,append=True)
                    stdouthandles.append(h)
                    stderrhandles.append(h)
                for file in std_prefixed:
                    prefixedhandles.append(open_or_handle(file))
                for file in std_prefixed_append:
                    prefixedhandles.append(open_or_handle(file,append=True))
                ...
                if onerror == Invoke.EXCEPTION:
                    ...
                    p = subprocess.Popen(cmdline, cwd=cwd, env=env, stdout=stdout_mode,
                                         stderr=stderr_mode, universal_newlines=True, bufsize=1, shell=shell)
                else:
                    try:
                ...
                if exitcode:
                    ...
                    if onerror == Invoke.ABORT:
                        ...
                        raise subprocess.CalledProcessError(
                            returncode=exitcode, cmd=shjoin(cmdline))
        ...
        if not forcepopen \
                ...
                and not stdout and not stderr and not std_joined and not std_prefixed and not stdout_append and not stderr_append and not std_joined_append and not std_prefixed_append :
            return execute_run()
    ...
    def diag(self, onerror=None, print_stdout=False, print_stderr=False, print_prefixed=True, print_command=True, print_exitcode=True, **kwargs):
        return self.execute(onerror=first_defined(onerror, Invoke.IGNORE), print_stdout=print_stdout, print_stderr=print_stderr, print_command=print_command, print_prefixed=print_prefixed, print_exitcode=print_exitcode, **kwargs)

    # Like a function call, not visible to the user
...
def query(cmd, *args, cwd=None, setenv=None, appendenv=None, **kwargs) -> str :
