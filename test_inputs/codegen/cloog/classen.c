if (m >= 1) {
  ...
  for (int c0 = 1, c0_end = 2 * m - 3; c0 < c0_end; c0 += 1) {
    if (c0 + 1 == m) {
    ...
    for (int c1 = max(2, -m + c0 + 3), c1_end = min(m - 1, c0); c1 <= c1_end; c1 += 1) {
    ...
    for (int c2 = max(1, -m + c0 + 2), c2_end = min(m, c0 + 1); c2 <= c2_end; c2 += 1)
      S8(c0, c2);
  }
  if (m >= 2) {
    if (m >= 3) {
    ...
    for (int c2 = m - 1, c2_end = m; c2 <= c2_end; c2 += 1)
      S8(2 * m - 3, c2);
